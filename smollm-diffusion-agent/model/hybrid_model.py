...
# Add parent directory to path for device_utils import
...
class HybridSmolLM(nn.Module):
    def __init__(self, base_model_id="HuggingFaceTB/SmolLM3-3B", load_in_4bit=False,
        ...
        if load_in_4bit:
            # Check if we're on CUDA (bitsandbytes only works on CUDA)
            if torch.cuda.is_available():
                ...
                # On Mac (MPS) or CPU, use float16/bfloat16 instead of 4-bit
                # bitsandbytes doesn't support MPS
                device = get_device()
                self.base_llm = AutoModelForCausalLM.from_pretrained(
                    base_model_id,
